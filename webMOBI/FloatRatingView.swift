...
public class FloatRatingView: UIView {
    ...
    func handleTouchAtLocation(touchLocation: CGPoint) {
        ...
        for i in stride(from: (self.maxRating-1), through: 0, by: -1){ //(self.maxRating-1){  for i in (self.maxRating-1).stride(through: 0, by: -1) {
    ...
    override public func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        // Update delegate
        if let delegate = self.delegate {
            delegate.floatRatingView(ratingView: self, didUpdate: self.rating)      //floatRatingView(self, didUpdate: self.rating)
        }
    }
