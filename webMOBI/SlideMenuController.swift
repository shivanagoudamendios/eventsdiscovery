...
public struct SlideMenuOptions {
    ...
    public static var rightViewWidth: CGFloat = 230.0
...
open class SlideMenuController: UIViewController, UIGestureRecognizerDelegate {
    ...
    open func initView() {
        ...
        if leftViewController != nil {
            ...
            if UIDevice.current.userInterfaceIdiom == .pad {
                leftFrame.size.width = SlideMenuOptions.leftViewWidth+50
            }
            else {
        ...
        if rightViewController != nil {
            ...
            if UIDevice.current.userInterfaceIdiom == .pad {
                rightFrame.size.width = SlideMenuOptions.rightViewWidth+50
            }
            else {
    ...
    open func addLeftGestures() {
        ...
        if (leftViewController != nil) {
            if leftPanGesture == nil {
                leftPanGesture = UIPanGestureRecognizer(target: self, action: #selector(SlideMenuController.handleLeftPanGesture))
    ...
    open func addRightGestures() {
        ...
        if (rightViewController != nil) {
            if rightPanGesture == nil {
                rightPanGesture = UIPanGestureRecognizer(target: self, action: #selector(SlideMenuController.handleRightPanGesture))
    ...
    fileprivate func leftMinOrigin() -> CGFloat {
        if UIDevice.current.userInterfaceIdiom == .pad {
            return  -(SlideMenuOptions.leftViewWidth+50)
        }
        else {
    ...
    fileprivate func isRightPointContainedWithinBezelRect(_ point: CGPoint) -> Bool {
        if let rightBezelWidth = SlideMenuOptions.rightBezelWidth {
            ...
            (tempRect, rightBezelRect ) = view.bounds.divided(atDistance: bezelWidth, from: .minXEdge)
...
extension UIViewController {
    ...
    func updatebudge()  {
        ...
        //        button.imageView?.tintColor = UIColor(hex: "2a2a2a")
        button.addTarget(self, action:#selector(UIViewController.toggleRight), for:.touchUpInside)
        
        let rightBarButton = BBBadgeBarButtonItem(customUIButton: button)
        
        //        let notifyvalue = UserDefaults.standard.integer(forKey: "requestnotification")
        
        rightBarButton?.badgeValue = "0"//notifyvalue.description
        
        
        
        rightBarButton?.badgeOriginX = 20
        rightBarButton?.badgeOriginY = 2
        rightBarButton?.badgeMinSize = 10
        
        self.navigationItem.rightBarButtonItem = rightBarButton
    }
    
    func budgeWithNotification(_ notification: Notification){
        updatebudge()
    }
    
    public func addRightBarButtonWithImage(_ buttonImage: UIImage) {
        updatebudge()
        NotificationCenter.default.addObserver(self, selector: #selector(self.budgeWithNotification), name:NSNotification.Name(rawValue: "budgeNotification"), object: nil)
